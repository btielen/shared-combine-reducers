...
import sharedCombineReducers  from '../src/sharedCombineReducers'

describe('Utils', () => {
  describe('combineReducers', () => {
    it('returns a composite reducer that maps the state keys to given reducers', () => {
      const reducer = sharedCombineReducers({
    ...
    it('warns if a reducer prop is undefined', () => {
      ...
      let isNotDefined
      sharedCombineReducers({ isNotDefined })
      expect(spy.mock.calls[0][0]).toMatch(
        /No reducer provided for key "isNotDefined"/
      )

      spy.mockClear()
      sharedCombineReducers({ thing: undefined })
      expect(spy.mock.calls[0][0]).toMatch(
    ...
    it('only warns for unexpected keys once', () => {
      ...
      expect(spy.mock.calls.length).toBe(0)
      const reducer = sharedCombineReducers({ foo, bar })
